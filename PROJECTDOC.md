...
| **Media** | Grid, browse all | Upload | — (delete + re-upload) | Delete | Images + video; local-disk storage adapter pending a real provider (§6 Q4) — see §5.8. Not wired into the HUD editor's URL fields; copy/paste the URL over |
...
- **Real database**: `prisma/schema.prisma` (Prisma ORM, SQLite for local dev — see §5.3). Every admin route queries it — the mock data layer (`server/utils/mockDb.ts`) that stood in during Phase 0-3 has been fully migrated off of and deleted (see §5.6).
- **Auth**: session lookups (`login`/`session`) query the real `AdminUser` table; the password check itself is still a hardcoded mock constant since there's no credential system yet. A minimal cookie session gates `/admin/*` behind a login page today. This is **not** production auth — it exists so the workflow (login → protected dashboard → logout) is real and demonstrable, and gets replaced once the client confirms a real auth strategy.
...
### 5.4 Mock → real database migration (complete)
`server/utils/mockDb.ts`'s generic helpers (`list`, `get`, `create`, `update`, `remove`) were always meant to be a swappable stand-in — see the original decision log in `DIARY.md` Day 1. That swap is done: every module was cut over from `mockDb` to `prisma` (via `server/utils/prisma.ts`, a singleton client) one at a time, same reference-module-first approach as the original UI build, and `mockDb.ts` has been deleted.

### 5.5 Reference CRUD module
**Categories** was built first as the complete, working pattern (list table, create/edit modal form, delete with confirm, server routes) precisely because it's the simplest entity in the model — and for the same reason, it was the first module cut over to the real database, proving the schema end-to-end before every other module followed the same mechanical migration.
...
### 5.8 Media & file storage

- **`MediaAsset` model** (`prisma/schema.prisma`): tracks every uploaded file. `ownerType`/`ownerId` are a loose, unenforced reference rather than a hard relation — HUD situations/screens/pop-ups already store their image/video as a plain URL string, so a media asset's "owner" is a label for browsing later, not a constraint.
- **Storage adapter** (`server/utils/storage.ts`): `saveFile()`/`deleteFile()` are the only two functions any caller touches. Local disk today (`public/uploads/`, gitignored) — zero setup, no account needed, same "don't block on a missing decision" approach used for the database. **Not production-appropriate as-is**: `public/` ships with the app bundle, so uploads don't survive a redeploy on most hosts. Swapping to S3/Cloudflare R2/Supabase Storage once the client answers §6 Q4 means replacing these two functions only — nothing else in the codebase changes.
- **Not wired into the HUD nested editor**: the Media library and the HUD editor's image/video URL fields are separate — "using" an uploaded asset means copying its URL from the Media grid into the HUD editor by hand. A picker that lets you select from the library directly is a reasonable future improvement, not required for the module to be functional.

### 5.9 Visual design system

The client sent reference mockups (`doc/IMG_2026090*.jpg`) after seeing the functionally-complete Phase 0–4 build and asked for a closer visual match before deploying anything live. Every admin page now follows this system:

- **Colorful gradient KPI cards** (`AdminStatCard.vue`) — fixed vivid colors (blue/green/purple/orange/red), not derived from the neutral theme tokens the rest of the console uses. See `app/utils/chartColors.ts` for why a separate, deliberately vivid palette is correct here rather than trying to force the reference's bold dashboard look out of a neutral admin theme.
- **Real charts** via Chart.js (`AdminChartCanvas.vue`, a single generic wrapper for line/bar/doughnut) — replaced the Phase 3 hand-rolled flat-div bar chart, which couldn't produce the reference's gradient area charts or donuts. `useChartTheme.ts` makes every chart's text/grid colors explicitly light/dark aware rather than relying on Chart.js's default gray, which reads dim on a dark background.
- **Consistent list-page header**: search input + 1–2 filter dropdowns + a primary "Add New X" button, with real client-side filtering logic (not just cosmetic — every list page now genuinely filters by the criteria shown).
- **Master-detail pattern**: click a table row to open a detail panel below it (tabs like Overview/sub-resource/Edit), replacing modal-only editing on Categories, Users, Referrals, and Orders. HUD Products deliberately keeps its existing single-scroll nested editor instead of adopting the reference's tab layout — see DIARY.md for why that's a considered exception, not an oversight.
- **Honest data only**: where the reference mockup shows a metric this system has no real source for (site-traffic-based conversion rate, a payment gateway's success rate, a "payment method" column with no processor wired up), it's omitted rather than faked. Real database data only, even under a firm "match this reference" instruction — see DIARY.md for the full reasoning.
- Two small, genuinely useful schema additions came out of chasing the reference honestly rather than superficially: `Category` gained `description`/`status`/`createdAt` (real reference columns, not just decoration), and `Setting` gained a License Settings section (new keys only, no migration — the store was already flexible key-value).
...
4. **File storage**: ~~Where do HUD images/videos live?~~ Real Media module built on a local-disk placeholder (§5.8) — works today, but doesn't survive a redeploy. Still need a real answer for production: S3, Cloudflare R2, or Supabase Storage all work with a small adapter swap.
...
| **Phase 1 — Core CRUD** | ✅ Users CRUD. ✅ HUD Products CRUD (incl. situations/screens/pop-ups nested editing). ✅ Media upload UI, on a local-disk placeholder pending a real storage provider decision (§6 Q4, §5.8). | Days 3–5 |
...
| **Phase 4 — Real backend cutover** | ✅ Database schema built and running (Prisma + SQLite, §5.3). ✅ Every module migrated from the mock layer to Prisma (§5.6) — `mockDb.ts` deleted. ⬜ Real auth/storage/payments — still blocked on client answers in §6. | Schema + migration done; auth/storage/payments pending client answers |
...
- **2026-09-07** — RBAC built same day. Along the way found a real security gap: the API layer had zero session verification (any direct request to any admin mutation endpoint worked with no cookie at all) — fixed with `server/middleware/admin-session.ts`, now the authoritative auth check (§5.7). Role permission matrix defined and enforced (`server/utils/permissions.ts`), Staff Accounts module added so roles are actually usable/testable, every audit log entry now attributes to the real actor instead of a hardcoded id.
- **2026-09-07** — Media module built same day (§5.8): `MediaAsset` schema + a local-disk storage adapter + a real upload/browse/delete UI. **Every module in §3.2 now has real, working functionality** — nothing left in the client's spec is still a placeholder. What's still genuinely open is all in §6: production hosting, real auth/payments/storage providers, referral commission logic, data privacy rules, the logo asset, and (separately, raised with the client directly) who owns the storefront build.
- **2026-09-07** — Before deploying a live preview, the client sent reference mockups and asked for a closer visual match. Full UI/UX redesign built same day (§5.9): real Chart.js charts, colorful gradient stat cards, a consistent search/filter/Add header, and a master-detail pattern across Categories/Users/Referrals/Orders. Two small schema additions (`Category.description/status/createdAt`, new `Setting` keys for License Settings) came out of matching the reference honestly. Deliberately did not fabricate metrics the system has no real source for (conversion rate, payment-gateway success rate) even though the reference mockup shows them. Next: get client sign-off on the visual direction, then the live preview deployment.
